...
function page_load(){
	...
	//parse featured categories
	var categories = JSON.parse($("#data-categories").val());
	categories.sort(function(a, b){return 0.5 - Math.random()});
	var cats = [];
	for(i=0;i<categories.length;i++){
		if(i == 10){break;}
		var cat = categories[i];
		var htm = '<div onclick="visit_category(\''+cat.category+'\')">'+cat.category+'</div>';
		cats.push(htm);
	}
	$("#featured").html(cats.join(""));
...
function search(inv){
	if(inv == ""){
		...
		$("#search_results").html("<i class='fa fa-spinner fa-pulse'></i> loading results...");
		$.ajax({
			type:'GET',
			url:'/home/search/'+inv,
			success:function(data){
				if(data.err){
					$("#search_results").html("<span class='text-danger'><i class='fa fa-warning'></i> failed to fetch results...</span>");
				}
				else{
					var res = data.result;
					var resu = [];
					for(i=0;i<res.length;i++){
						var re = res[i];
						var htm = '<a href="/item/'+re.id+'" class="search_result">'+re.title+' <span>('+re.category+' | '+re.subcategory+')</span></a>';
						resu.push(htm);
					}
					$("#search_results").html(resu.join(""));
				}
			},
			error:function(){
				$("#search_results").html("<span class='text-danger'><i class='fa fa-warning'></i> failed to fetch results...</span>");
